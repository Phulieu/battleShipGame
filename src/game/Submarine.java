...
public class Submarine extends Ship{
     ...
     * @param sName
     */
    public Submarine(ShipType[] sPreferredTarget, String sName) {
        super((int) (Math.random()*25+50),100,15,10,sPreferredTarget);
        this.sName = sName;
    }
    @Override
    /**
    ...
    public ShipType pickTarget(){
        ...
        //in range 5%
        if (randomPro <=5) return sPreferredTarget[0];
            //in range 10% next
        else if ( randomPro <=15 ) return sPreferredTarget[1];
            //in range 75% next
        else if ( randomPro <=90 ) return sPreferredTarget[2];
            //in range 5% next
